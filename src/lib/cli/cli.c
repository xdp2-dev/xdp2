...
void cli_set_prompt_start_color(struct cli_def *cli, const char *string) {
  free_z(cli->prompt_start_color);
  if (string && *string) cli->prompt_start_color = strdup(string);
}

void cli_set_prompt_stop_color(struct cli_def *cli, const char *string) {
  free_z(cli->prompt_stop_color);
  if (string && *string) cli->prompt_stop_color = strdup(string);
}
...
static int show_prompt(struct cli_def *cli, int sockfd) {
  ...
  if (cli->prompt_start_color) len += write(sockfd, cli->prompt_start_color,
					    strlen(cli->prompt_start_color));
  ...
  len += write(sockfd, cli->promptchar, strlen(cli->promptchar));
  if (cli->prompt_stop_color) len += write(sockfd, cli->prompt_stop_color,
					    strlen(cli->prompt_stop_color));
  return len;
}
...
/* GCC doesn't like using non-literals as format to printf. Disable the
 * compiler warning here for vasprintf to suppress a bunch of warnings
 */
  ...
  do {
    ...
    if (print) {
      if (cli->print_callback)
        cli->print_callback(cli, p, need_newline);
      else if (cli->client) {
	 if (need_newline)
...
void cli_print_callback(struct cli_def *cli, void (*callback)(struct cli_def *, const char *, int)) {
...
void cli_int_buildmode_reset_unset_help(struct cli_def *cli) {
  ...
  if (cmd) {
    ...
    if (optarg) {
      ...
      struct cli_optarg_pair *optarg_pair;
      /*
       * This will ensure that any previously added help is not propagated - this left over space will be freed by the
...
static void cli_get_optarg_comphelp(struct cli_def *cli, struct cli_optarg *optarg, struct cli_comphelp *comphelp,
  ...
  if (lastchar == '?') {
     ...
     *  wrap on nearest preceding whitespace when it hits a boundary.  Subsequent lines will be indented
