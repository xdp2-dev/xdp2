...
**Packet IO vector buffers**, or PVbufs, are data structures the hold arbitrary 
packet data. A PVbuf is a structure than contains an array of iovecs, and the
iovecs support scatter/gather semantics. Each iovec is a **paddr** that is a
specialized memory address. There are a few different types of paddrs to
address diffent object types. One paddr type holds the address of a PVbuf.
An iovec in a PVbuf may be set to the paddr for aother PVbuf thereby allowing
...
in number of cachelines (64 bytes) minus one. The index is the index of
...
The minimum length PVbuf is 64 bytes (eight iovecs) and the maximum is 1024
bytes or 256 iovecs.
...
and if a bit is not set then the corresponging iovec is NULL. The bitmap
facilitates a fast scan of non-NULL iovecs, and provides for efficient
initialzation of the PVbuf by just zero'ing the bitmap. Note that the
...
zero which indicates no length is reported pvbufs as packet lists
...
Packet buffers, or pbufs, are memory buffers in memory that hold any sort of
packet or object data. Packet buffers are allocated from contiguous address
...
The paddr type differentiates different types of addresses. To indicate the
address is for a pbuf, the paddr type is set to 0b0010. The four bit buffer
tag indicates the object size of the pbuf referred to by the address. The
object size is 2**N where N=buffer_tag + 6 and buffer_tag is a number between
0 and 14 inclusive. The semantics of buffer tag 15 are described below. The
pbuf index identifies a pbuf; this is the index into the array of pbufs for a
particular size managed by an object allocator. The combination of the buffer
tag and pbuf index uniquely identify a pbuf. The pbuf offset is a byte offset
in the referenced pbuf; this allows addressing specific bytes in a pbuf.

Buffer tag 15 is mostly a synonym for buffer tag 14. When buffer tage 15 is
...
allows 48TBytes of addressible memory. The data length is set to the real
...
sixth bit inidicates the first or second word of a long address paddr. The
...
xdp2_paddr_t xdp2_pbuf_alloc(struct xdp2_pvbuf_mgr *pvmgr,
			     size_t size, void **addr)
```
*size* is the requested size and *addr* is used to return an absolute
pointer to the pbuf.  A padd  is returned or a NULL paddr upon allocation
...
This variant allocates a pbuf but sets the offset so that the data is at
the tail of the allocated buffer
...
prepending the data. If the operation was successful then true in the *paddr*
PVbuf is unknown. If the operation succeeds then true is returned
...
and *compress* indicates whether the pvbuf should be compressed after dropping
bytes. *copyptr* is space to copy the dropped bytes, if it is NULL then
the data is not copied. The return value is the number of bytes popped, it
may be less than *num_drop_bytes* if there length of data in *pvbuf* is less
than *num_drop_bytes*

Append trailers to a pvbuf
--------------------------
...
bool __xdp2_pvbuf_append_pvbuf(struct xdp2_pvbuf_mgr *pvmgr,
                               ...
                               size_t length, bool compress)
```
*paddr* is the target pvbuf, *trlers* is the input pvbuf that will be
...
Arguments are:
  ...
  * **-v**: Verbosity level. Higher number produces more output
  ...
  * **-N <num-test-bufferd>: Number of test buffers
...
interation by defualt

```
./test/pvbuf/test_pvbuf
```

Run a single pvbuf through 1000 random operations with test output at the
...
C
Run 10,000,000,000 operations  with maximum pvbuf size of 1,000,000 bytes
Run 1,000,000 operations with maximum pvbuf size of 10,000 bytes, five
test pvbufs, and compression enabled. Configure number of buffers for object
allocators so with only twenty sixty-four byte buffers and fifteen pvbufs to
force object allocation failure.

```

./test/pvbuf/test_pvbuf -c 1000000 -I 100000 -M 10000 -A 0:20 -A 1:1000000 -A 4:10000 -A 8:2000 -A 6:4000 -N 5 -P 15 -v 5 -C
Buffer manager at start
global-pvbuf-alloc:: obj_size: 64, max_objs: 15, num_free: 15, num_allocs: 0
global-pbuf-alloc-0:: obj_size: 64, max_objs: 20, num_free: 20, num_allocs: 0
global-pbuf-alloc-1:: obj_size: 128, max_objs: 1000000, num_free: 1000000, num_allocs: 0
global-pbuf-alloc-4:: obj_size: 1024, max_objs: 10000, num_free: 10000, num_allocs: 0
global-pbuf-alloc-6:: obj_size: 4096, max_objs: 4000, num_free: 4000, num_allocs: 0
global-pbuf-alloc-8:: obj_size: 16384, max_objs: 2000, num_free: 2000, num_allocs: 0
Iter: 0
Iter: 100000
Iter: 200000
Iter: 300000
...
Operation counts:
	pop headers: 125338
	prepend headers: 118715
	append trailers: 124485
	pop trailers: 125179
	push pbuf headers: 125047
	push pbuf trailers: 125203
	pullup headers: 124988
	pullup trailers: 125139
Buffer manager at end
global-pvbuf-alloc:: obj_size: 64, max_objs: 15, num_free: 15, num_allocs: 327725
global-pbuf-alloc-0:: obj_size: 64, max_objs: 20, num_free: 20, num_allocs: 165400
global-pbuf-alloc-1:: obj_size: 128, max_objs: 1000000, num_free: 1000000, num_allocs: 190531
global-pbuf-alloc-4:: obj_size: 1024, max_objs: 10000, num_free: 10000, num_allocs: 187088
global-pbuf-alloc-6:: obj_size: 4096, max_objs: 4000, num_free: 4000, num_allocs: 181033
global-pbuf-alloc-8:: obj_size: 16384, max_objs: 2000, num_free: 2000, num_allocs: 74455
Fail counts: 
	prepend: 73
	prepend_pbuf: 50
	pullup: 3
...
Note that at the end of the test, all the pbufs and pvbufs are free (num_free 
value equals max_objs) and all the fail counts are non-zero which is
expected
